...
| `{{METRICS_ROWS}}` | HTML | **必须 9 行 `<tr>`，每行 4 个 `<td>`**。首列**只能**是这 9 个比率名（按此顺序）：`毛利率`、`营业利润率`、`净利率`、`ROE`、`ROA`、`资产负债率`、`利息保障倍数`、`每股收益（EPS）`（接受 `稀释EPS` 别名）、`自由现金流利润率`。**不得**把营业收入 / 毛利润 / 营业利润 / 净利润 / 经营现金流 / 自由现金流的**绝对金额**写进此表（那些已在 Sankey 与 KPI/趋势卡呈现）。当某比率因资产负债表字段缺失算不出（如 `total_equity` 缺致 ROE 不可计算），首列仍写比率名，第二、三列写 `不适用`，**不得**整行省略。第四列 **`同比变动` 必须是结论性定调**，不是裸数字或百分比，使用 `references/financial_metrics.md` 的受控词表：`显著改善`、`改善`、`基本持平`、`恶化`、`显著恶化`、`权益缺口收窄`、`权益缺口扩大`、`期末股东权益为负`、`不适用`；**不得**把 `+0.62%` / `-1.4pct` 塞进第四列。Anamnesis 主层 `validate_report_html.py` 对行数、首列白名单、`<td>` 个数、第四列受控词均 fail-closed 强制（incident I-005）。 |
