...
- **QC / deliberation transparency (encouraged):** When dual-QC resolution materially affects a Porter bullet, ground the wording in the **real** merge result in `qc_audit_trail.json` and finalized `porter_analysis.json`. Distinguish clearly between **reasoning changes with no score move** and **actual score changes**. The full audit trail remains in `qc_audit_trail.json` and appendix / `qc_deliberation` summaries.
- **When QC keeps the score unchanged**, you may use a short maintained-score opening such as:  
  **"Dual-QC deliberation maintained supplier power at 3/5. …"** or **"After dual-QC deliberation, supplier power remains 3/5. …"** Then continue with evidence and analysis.
- **When QC changes a radar score** (vs the first draft), use this recommended skeleton inside the matching `<li>` (wording may vary; keep all four elements):  
  **"Dual-QC deliberation held that …, and adjusted the [force] score from *a* to *b*, because …; …"** Then continue with the usual evidence and analysis. Use integers **1–5** for *a* and *b*.  
  **Do not fabricate a prior score** for stylistic symmetry. If the audit trail does not prove a real score change, write the maintained-score form instead.
- **Source text:** `porter_analysis.json` fields under `company_perspective`, `industry_perspective`, and `forward_perspective` should store lines that map cleanly into the five `<li>` elements; if scores moved after QC, the same from–to content should appear in JSON and HTML, and if scores did **not** move, both JSON and HTML should consistently use a maintained-score narrative.
